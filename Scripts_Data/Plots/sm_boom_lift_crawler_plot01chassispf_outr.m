...
if ~exist('h1_sm_boom_lift_crawler', 'var') || ...
        ~isgraphics(h1_sm_boom_lift_crawler, 'figure')
    h1_sm_boom_lift_crawler = figure('Name', 'sm_boom_lift_crawler');
end
figure(h1_sm_boom_lift_crawler)
clf(h1_sm_boom_lift_crawler)
...
grid on
ylabel('Extension (in)')
title('Chassis Motion: Outriggers')
legend({'Outrigger FL','Outrigger FR','Outrigger RL','Outrigger RR'},'Location','Best');

simlog_handles(2) = subplot(2, 1, 2);
plot(simlog_t, simlog_OutFLf, 'LineWidth', 1)
hold on
plot(simlog_t, simlog_OutFRf, 'LineWidth', 1)
plot(simlog_t, simlog_OutRLf, 'LineWidth', 1)
plot(simlog_t, simlog_OutRRf, 'LineWidth', 1)
hold off
...
maxF = max(abs([simlog_OutFLf;simlog_OutFRf;simlog_OutRLf;simlog_OutRRf]));
if(maxF == 0)
    set(gca,'YLim',[-1 1])
else
