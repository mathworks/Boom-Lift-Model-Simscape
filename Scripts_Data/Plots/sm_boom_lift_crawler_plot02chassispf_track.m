...
if ~exist('h2_sm_boom_lift_crawler', 'var') || ...
        ~isgraphics(h2_sm_boom_lift_crawler, 'figure')
    h2_sm_boom_lift_crawler = figure('Name', 'sm_boom_lift_crawler');
end
figure(h2_sm_boom_lift_crawler)
clf(h2_sm_boom_lift_crawler)

simlog_handles(1) = subplot(2, 1, 1);
plot(simlog_t, simlog_TrackLp*100/2.54, 'LineWidth', 1)
hold on
plot(simlog_t, simlog_TrackRp*100/2.54, 'LineWidth', 1)
hold off

grid on
ylabel('Extension (in)')
title('Chassis Motion: Tracks')
legend({'Track L','Track R'},'Location','Best');

simlog_handles(2) = subplot(2, 1, 2);
plot(simlog_t, simlog_TrackLf, 'LineWidth', 1)
hold on
plot(simlog_t, simlog_TrackRf, 'LineWidth', 1)
hold off
...
maxF = max(abs([simlog_TrackLf;simlog_TrackRf]));
if(maxF == 0)
